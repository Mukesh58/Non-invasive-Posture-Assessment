...
def main():
    ...
    alert_PostureThreshold = 5  # Number of consecutive frames in poor posture before poor posture event is triggered
    calib_framesRequired = 5  # Number of frames in confidence threshold to achieve satisfactory calibration
    ...
    while True:
        ...
        if frameNum % frameInterval == 0 and frameNum > 1 * fps:
            ...
            if boolUserInFrame == True and calibFlag == False:
                ...
                # print("(dist_noseNeck_y/calib_noseNeck_y)" + str((dist_noseNeck_y/calib_noseNeck_y)))

                #  Left minus Right: If >0, Left eye is lower than right
                offset_eyesLR_y = frameData[3][1] - frameData[4][1]

                currentPosture = 0  # Default posture which is 0 - healthy

                # Detect Forward Head
                # Distances: dist_noseNeck_y > 25.1825 and dist_eyeLNose_y < 6.0821 and dist_eyes_x >5.2467

                print(str(diff_nose_x / calib_nose[0]) + "    " + str(diff_eyeL_x / calib_eyeL[0]) + "    " + str(
                    diff_eyeR_x / calib_eyeR[0]) + "    " + str(dist_noseNeck_x))

                if (diff_nose_y / calib_nose[1]) > 0.1 and (diff_neck_y / calib_neck[1]) > 0.04 and (
                        diff_eyeL_y / calib_eyeL[1]) > 0.10 and (diff_eyeR_y / calib_eyeR[1]) > 0.10 and (
                        diff_shoulderL_y / calib_shoulderL[1]) > 0.05 and (
                        diff_shoulderR_y / calib_shoulderR[1]) > 0.05 and (
                        dist_shoulderBreadth / calib_shoulderBreadth) > 1.08 and (
                        dist_noseNeck_y / calib_noseNeck_y) > 0.83:
                    print("Shoulders Slouched")
                    currentPosture = 1
                elif (diff_nose_y / calib_nose[1]) > 0.12 and (diff_eyeL_y / calib_eyeL[1]) > 0.10 and (
                        diff_eyeR_y / calib_eyeR[1]) > 0.10 or (dist_eyes_x / calib_eyes_x) > 1.1 and (
                        dist_noseNeck_y / calib_noseNeck_y) < 0.83:
                    print("Forward Head")
                    currentPosture = 2
                elif (diff_eyeL_y / calib_eyeL[1]) > 0.03 and (
                        diff_eyeR_y / calib_eyeR[1]) < -0.03 and offset_eyesLR_y > 6:
                    ...
                    currentPosture = 4
                elif (diff_nose_x / calib_nose[0]) > 0.0798 and (diff_eyeL_x / calib_eyeL[0]) > 0.06 and (
                        diff_eyeR_x / calib_eyeR[0]) > 0.06 and dist_noseNeck_x > 30:
                    print("Head Rotated - Left")
                    currentPosture = 5
                elif (diff_nose_x / calib_nose[0]) < -0.07 and (diff_eyeL_x / calib_eyeL[0]) < -0.07 and (
                        diff_eyeR_x / calib_eyeR[0]) < -0.07 and dist_noseNeck_x < -30:
                ...
                if count_unhealthyPose > alert_PostureThreshold:
                    ...
                    Message(title="Alert!",
                            message="Unhealthy pose detected - " + postureStatus[currentPosture].strip(),
                            master=root).show()
                    root = tkinter.Tk()
                    root.withdraw()
                    count_poseEvent += 1
                    unhealthyPoseList.append(str(postureStatus[currentPosture]))
                    count_unhealthyPose = 0
                    time.sleep(2)
                    ...
                    img2 = cv2.putText(img, str(postureStatus[currentPosture]), (150, 100), font, 1,
                                       (255, 255, 255), 2)
                    img_name = check_fileName(
                        "img/" + start_time + "_PoseDetected_" + postureStatus[currentPosture].strip() + ".png")
            ...
            # Branch runs if user is in frame and program in initial calibration sequence
                ...
                if (frameData[1][2] > 0.75 and frameData[2][2] > 0.625 and frameData[3][2] > 0.825 and frameData[4][
                    2] > 0.825):
            ...
            if (calibFlag):
                img = cv2.putText(img,
                                  "Calibration in Progress: " + str(calibCounter) + "/" + str(calib_framesRequired),
                                  (100, 50), font, 1, (255, 0, 0), 3)
            ...
            if key == ord('q') or frameNum == 100000:  # Press key to end program
                ...
                cv2.imwrite(img_name, img)
                break
            elif key == ord('s'):  # Save image capture
                print("--------------------Image Saving-----------------")
                img_name = check_fileName("img/" + start_time + "_Data.png")
                cv2.imwrite(img_name, img)
            elif key == ord('r'):  # Initiate recalibration manually
                calibFlag = True
                calibCounter = -10
                calibData = []
    ...
    for j in range(0, df.shape[1]):
        for i in range(0, df.shape[0] - window_size + 1):
            df_sma.loc[df_sma.index[i + window_size - 1], str(df_sma.columns.values[j])] = np.round(((df.iloc[i, j] +
                                                                                                      df.iloc[
                                                                                                          i + 1, j] +
                                                                                                      df.iloc[
                                                                                                          i + 2, j] +
    ...
    if len(unhealthyPoseList) > 0:
        f.write(str(start_time) + "," + str(count_poseEvent) + ",")
